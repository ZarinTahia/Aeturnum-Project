...
from data.s3_storage import upload_from_url
...
# ── Step 2: Update blocks + files table ───────
...
for img in IMAGES:
    ...
    cur.execute("UPDATE blocks SET content = %s WHERE id = %s",
                (new_content, img["block_id"]))
    cur.execute("UPDATE files SET path = %s WHERE id = %s",
                (img["s3_path"], img["file_id"]))

conn.commit()
cur.close()
conn.close()
print("✅ S3 upload and database update complete.")
